...
def determine_turn_actions(current_heading, target_heading):
    ...
    while abs(angle_diff) >= 30 and large_rotation_count < MAX_LARGE_ROTATIONS: # 6
        ...
        large_rotation_count += 1
    # Small angle rotation (6°)
    while abs(angle_diff) >= 6 and small_rotation_count < MAX_SMALL_ROTATIONS and random.random() < SMALL_THRESHOLD:  # 2
    # while abs(angle_diff) >= 6 and small_rotation_count < MAX_SMALL_ROTATIONS: # 2
        if angle_diff > 0:
            actions.append(ACTIONS['rotate_right_small'])
            ...
            angle_diff += 6
        small_rotation_count += 1
    # print("small_rotation_count:", small_rotation_count)
    # print("large_rotation_count:", large_rotation_count)
    return actions, current_heading

def determine_forward_actions(current_heading, current_pose, distance, step_length=0.2):
    """
    ...
    new_x = current_pose['x'] + num_moves * step_length * math.sin(math.radians(current_heading))
    new_z = current_pose['z'] + num_moves * step_length * math.cos(math.radians(current_heading))
...
def plot_grid_map(shortest_path, actions, initial_pose, step_length=0.2, save_path=None, threshold=0.2, reachable_positions=None):
    """
    ...
    ax.plot(path_x, path_z, 'yo-', label='shortest path', markersize=3)
...
def shortest_path_to_actions(shortest_path, initial_pose):
    ...
    for i in range(len(shortest_path)):
        ...
        forward_actions, current_pose = determine_forward_actions(current_heading, current_pose, distance, step_length=0.2)
...
if __name__ == "__main__":
    ...
    # Visualization
    plot_grid_map(shortest_path, actions, initial_pose, save_path=".", step_length=0.2)