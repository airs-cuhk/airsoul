...
def sample_from_shortest_path(shortest_path):
    """
    ...
    def sample_line_segment(p1, p2):
        ...
        if distance < AGENT_MOVEMENT_CONSTANT:
            return [(x1,y1), (x2,y2)]
        # Number of sample points
        num_samples = int(distance / AGENT_MOVEMENT_CONSTANT)
...
def check(point, shortest_path, threshold=0.2):
    """
    ...
    """
    points = sample_from_shortest_path(shortest_path)
    _points = list(points)
    distances = []
...
def determine_turn_actions_(angle_diff):
    ...
    while abs(angle_diff) >= 30 and large_rotation_count < MAX_LARGE_ROTATIONS:  # 6
        if angle_diff > 0:
    ...
    while abs(angle_diff) >= 6 and small_rotation_count < MAX_SMALL_ROTATIONS:  # 2
        if angle_diff > 0:
...
def determine_turn_actions(current_heading, target_heading):
    ...
    while abs(angle_diff) >= 30 and large_rotation_count < MAX_LARGE_ROTATIONS: # 6
        ...
        large_rotation_count += 1
    # Small angle rotation (6°)
    while abs(angle_diff) >= 6 and small_rotation_count < MAX_SMALL_ROTATIONS and random.random() < SMALL_THRESHOLD:
        if angle_diff > 0:
            actions.append(ACTIONS['rotate_right_small'])
...
def determine_forward_actions(distance, step_length=0.2):
    """
    ...
    """
    actions = []
    num_moves = int(distance / step_length)
    actions.extend([ACTIONS['move_ahead']] * num_moves)
    
    # Handle remaining distance
    remaining_distance = distance - (num_moves * step_length)
    if remaining_distance >= (step_length / 2):
        actions.append(ACTIONS['move_ahead'])
    
    return actions

def plot_grid_map(shortest_path, actions, initial_pose, step_length=0.2, save_path=None, threshold=0.2):
    """
    ...
    pose_sequence = execute_actions(actions, initial_pose, step_length)
    ...
    for idx, pose in enumerate(pose_sequence):
        ...
        if not check((pos['x'], pos['z']), shortest_path):
            circle = Circle((pos['x'], pos['z']), radius=threshold, color='g', fill=True)
            ax.add_patch(circle)
        # Draw heading arrow
        ...
        # Circle sample 
        circle = Circle((pos['x'], pos['z']), radius=threshold, color='r', fill=False)
        ax.add_patch(circle)
...
def execute_actions(actions, initial_pose, step_length=0.2):
    """
    ...
    for action in actions:
        ...
        if action == ACTIONS['move_ahead']:
            ...
            dx = step_length * math.sin(radians)
            dz = step_length * math.cos(radians)
            ...
            # Rotate left 6 degrees
            current_heading = (current_heading - 6) % 360
            current_pose['rotation']['y'] = current_heading

        elif action == ACTIONS['rotate_right_small']:
            # Rotate right 6 degrees
            current_heading = (current_heading + 6) % 360
            current_pose['rotation']['y'] = current_heading
...
def shortest_path_to_actions(shortest_path, initial_pose):
    ...
    for i in range(len(shortest_path) - 1):
        current_point = shortest_path[i]
        next_point = shortest_path[i + 1]
        current_pos = {'x': current_point['x'], 'z': current_point['z']}
        next_pos = {'x': next_point['x'], 'z': next_point['z']}

        # Calculate target angle
        target_angle = calculate_angle(current_pos, next_pos)
        ...
        forward_actions = determine_forward_actions(distance, step_length=0.2)
...
if __name__ == "__main__":
    ...
    shortest_path = [
        {'x': 8.25, 'y': 0.07260513305664062, 'z': 3.75},
        {'x': 6.75, 'y': 0.07260513305664062, 'z': 3.75},
        {'x': 4.6666669845581055, 'y': 0.07260513305664062, 'z': 1.5},
        {'x': 4.333333492279053, 'y': 0.07260513305664062, 'z': 1.1666667461395264},
        {'x': 3.3333334922790527, 'y': 0.07260513305664062, 'z': 1.1666667461395264},
        {'x': 3.1499972343444824, 'y': 0.07260513305664062, 'z': 1.1999990940093994}
    ]
    ...
    # Visualization
    plot_grid_map(shortest_path, actions, initial_pose, save_path="./debug/test-actions.png",step_length=0.2)