...
class E2EObjNavSA(nn.Module):
    ...
    def sequential_loss(self, prompts, 
        ...
        #         # print(loss_weight_a.shape, self.loss_weight[ps:pe].unsqueeze(0).shape, ps, pe)
        #         loss_weight_a = loss_weight_a * self.loss_weight[ps:pe].unsqueeze(0)
        #         loss_weight_s = loss_weight_a.clone()
        #         # print(loss_weight_a.shape, loss_weight_s.shape)
...
class E2EObjNavDinoV2(nn.Module):
    def __init__(self, config, verbose=False): 
        super().__init__()
        self.config = config
        # self.feat_encoder = MLPEncoder(config.feat_encoder_block)
        # self.feat_decoder = MLPEncoder(config.feat_decoder_block)
        self.decision_model = POTARDecisionModel(config.decision_block)
        ...
        if(verbose):
            print("E2EObjNavSA initialized, total params: {}".format(count_parameters(self)))
            print("decision_model_params: {}".format(
                # count_parameters(self.feat_encoder), 
                # count_parameters(self.feat_decoder), 
                count_parameters(self.decision_model)))
    ...
    def forward(self, observations, 
        ...
        B = actions.shape[0]
        NT = actions.shape[1]
        # z_rec = self.feat_encoder(observations)
        # z_rec = z_rec.view(B, NT, -1)
        wm_out, pm_out, new_cache = self.decision_model(
                observations, prompts, tags, actions, rewards,
                cache=cache, need_cache=need_cache, state_dropout=state_dropout, 
                update_memory=update_memory)

        return observations, wm_out, pm_out, new_cache
    ...
    def sequential_loss(self, prompts, 
        ...
        # self.feat_encoder.requires_grad_(True)
        ...
        if not self.config.decision_block.state_diffusion.enable:
            ...
            # World Model Loss - Raw Image
            # obs_pred = self.feat_decoder(z_pred)
            obs_pred = z_pred
            loss["wm-raw"] = weighted_loss(obs_pred, 
                                        loss_type="mse",
                                        gt=inputs[:, 1:], 
        ...
        if self.action_dtype == "Discrete":
            if(self.policy_loss == 'crossentropy'):
                ...
                loss["pm"], loss["count_pm"] = weighted_loss(a_pred,
                                        ...
                                        reduce_dim=reduce_dim,
                                        need_cnt=True)
                # print(loss["pm"].shape, loss["count_pm"], loss_weight_a.shape)
            elif(self.policy_loss == 'mse'):
                if(use_loss_weight):
                    loss_weight_a = self.loss_weight[ps:pe]
                else:
        ...
        return loss, z_pred, a_pred, cache
    ...
    def preprocess_observation(self, 
        ...
        if(single_batch and obs.dim() < 3):
            obs = obs.unsqueeze(0)
        if(single_step and obs.dim() < 3):
            obs = obs.unsqueeze(1)
        ...
        # else:
        #     z_rec = obs
        # convert torch tensor into .dtype(torch.float32)
        z_rec = obs
        z_rec = z_rec.to(torch.float32)
        return z_rec
