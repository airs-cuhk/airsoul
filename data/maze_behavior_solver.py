...
class MazeQuietExpertAgent(SmartSLAMAgent):
    def __init__(self, 
                 memory_keep_prob_range=[0.05, 1.0],
                 horizon=3000, # Noise decay factor
                 **kwargs):
        super().__init__(**kwargs)
        self.noise_initial = 0
        self.noise_final = 0
        self.memory_keep_ratio = 1 # random.uniform(*memory_keep_prob_range)
        self.steps = 0
...
class MazeNoisyExpertAgent(SmartSLAMAgent):
    def __init__(self, 
                 noise_initial_range=[0, 1],
                 noise_final_range=[0, 1],
                 memory_keep_prob_range=[0.05, 1.0],
                 horizon=3000, # Noise decay factor
                 **kwargs):
        super().__init__(**kwargs)
