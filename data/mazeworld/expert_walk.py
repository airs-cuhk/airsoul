...
def run_maze_epoch(
    ...
    # actions id: 0 - 15 
    expert_agent = SmartSLAMAgent(maze_env=maze_env, render=False, short_term_memory_size = 5, memory_keep_ratio=1)
...
def dump_maze(work_id, path_name, traj_folders, memory_keep_ratio, max_steps):
    ...
    for idx in range(len(traj_folders)):
        ...
        out_root = "/pfs/pfs-r36Cge/zsj/yuxuan/airsoul-diffusion/0707-expert100-walk"
        file_name = traj_folders[idx].split("/")[-1]
        ...
        maze_env.save_trajectory(os.path.join(file_path, f"expert100_trajectory1.png"))
        re_path = os.path.join(file_path, f"expert100_rewards1.npy")
        numpy.save(re_path, results["rewards"])
        print("Saved rewards to %s" % re_path)
        cmd_path = os.path.join(file_path, f"expert100_commands1.npy")
        numpy.save(cmd_path, results["commands"])
        print("Saved commands to %s" % cmd_path)
        L_task_path = os.path.join(file_path, f"expert100_Ltask1.npy")
        numpy.save(L_task_path, results["L_task"])
        print("Saved L_task to %s" % L_task_path)

        # record_points = [100, 700, 1000, 2000, 3000, 8900, 9500]
        # maze_env = gym.make("mazeworld-v2", enable_render=False, max_steps=max_steps, resolution=(128, 128))
        # maze_env.set_task(task)
        # results = run_maze_epoch(
        #         maze_env,
        ...
        #     assert False
        # print("Saving trajectory to %s" % file_path)
        # maze_env.save_trajectory(os.path.join(file_path, f"expert5_trajectory.png"))
        # re_path = os.path.join(file_path, f"expert5_rewards.npy")
        # numpy.save(re_path, results["rewards"])
        # print("Saved rewards to %s" % re_path)
        # cmd_path = os.path.join(file_path, f"expert5_commands.npy")
        # numpy.save(cmd_path, results["commands"])
        # print("Saved commands to %s" % cmd_path)
        # L_task_path = os.path.join(file_path, f"expert5_Ltask.npy")
        # numpy.save(L_task_path, results["L_task"])
        # print("Saved L_task to %s" % L_task_path)        
