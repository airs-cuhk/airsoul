...
def change_action_refer(refer_action_path):
    ...
    refer_len = 11 # len(_actions_label[-1])
...
def one_target2maze(target_dir):
    ...
    # BEVs =_BEVs
    # BEVs = [_BEVs.copy() for _ in range(len(_observations))]  # FIXME: BEVs = [_BEVs]
    ...
    assert len(observations) == len(actions_behavior_id), f"O BA Subsequence Length mismatch: {len(observations)} != {len(actions_behavior_id)}"
    assert len(observations) == len(actions_behavior_val), f"O BAv Subsequence Length mismatch: {len(observations)} != {len(actions_behavior_val)}"
    assert len(observations) == len(actions_label_val), f"O LAv Subsequence Length mismatch: {len(observations)} != {len(actions_label_val)}"
    assert len(actions_behavior_id) == len(actions_label_id), f"O LA Subsequence Length mismatch: {len(actions_behavior_id)} != {len(actions_label_id)}"
    return (
        observations,
        actions_behavior_id,
...
def house_target2maze(
    ...
    #     return save_dir
    def get_target_id_as_save_dir(target_dir, save_root):
        target_id = target_dir.split("/")[-2:]
        target_id = "_".join(target_id)
        save_path = os.path.join(save_root, target_id)
    ...
    for target in os.listdir(house_dir):
        ...
        if os.path.isdir(target_dir):
            ...
            if save and len(_observations) > 0:
                # save_dir = get_house_id_as_save_dir(house_dir)
                save_dir = get_target_id_as_save_dir(target_dir, save_root=save_root)
        ...
        for house in tqdm(house_ids, desc=f"Processing houses in {subdir}"):
            ...
            if os.path.isdir(house_dir):
                (
                    ...
                    _target,
                    _house,
                ) = house_target2maze(house_dir, save_root=os.path.join(save_dir, subdir))
    return
    ...
    try:
        if os.path.isdir(house_dir):
            (
                ...
                _target,
                _house,
            ) = house_target2maze(house_dir, save_root=save_subdir)
    except Exception as e:
...
def process_subdir(subdir, houses_dir, save_dir):
    ...
    try:
        # house_ids = read_txt(file_path=os.path.join(sub_houses_dir, "success_room.txt"))
        house_ids = []
        for house_id in os.listdir(sub_houses_dir):
            if os.path.isdir(os.path.join(sub_houses_dir, house_id)):
                house_ids.append(house_id)
        print(f"Processing {len(house_ids)} houses in {subdir}")
...
if __name__ == "__main__":
    ...
    #                           save_dir = "/pfs/pfs-r36Cge/qxg/datasets/procthor/07-23-test",
    #                         #   num_processes=1,
    #                           )


    # get_all_house_target2maze_mutiprocess(houses_dir="/pfs/pfs-r36Cge/qxg/datasets/procthor/0721-train-10000-noise/train/", 
    #                           save_dir = "/pfs/pfs-r36Cge/qxg/datasets/procthor/07-24-behaviour_only_noblack",
    #                         #   num_processes=1,
    #                           )

    get_all_house_target2maze_mutiprocess(houses_dir="/pfs/pfs-r36Cge/qxg/datasets/procthor/07-21-Oracle-data/train/", 
                              save_dir = "/pfs/pfs-r36Cge/qxg/datasets/procthor/07-24-oracle_train_trajectory_short")
