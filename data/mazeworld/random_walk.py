...
def run_maze_epoch(
    ...
    # print("Memory keep ratio:", expert_agent.memory_keep_ratio)
     # actions id: 0 - 15 
    random_agent = lambda x: random.randint(0, 15)
    ...
    while not done:
        lact_id = random_agent(observation)
...
def dump_maze(work_id, path_name, traj_folders, memory_keep_ratio, max_steps):
    ...
    for idx in range(len(traj_folders)):
        ...
        maze_env.save_trajectory(os.path.join(file_path, f"random_trajectory.png"))
        re_path = os.path.join(file_path, f"random_rewards.npy")
        numpy.save(re_path, results["rewards"])
        print("Saved rewards to %s" % re_path)
        cmd_path = os.path.join(file_path, f"random_commands.npy")
